...
//按高度进行比例缩放，输入和输出都是图片字节数组
func ScaleB2Bh(InRaw []byte, height int) (OutRaw []byte, err error) {
	img, filetype, err := Scaleh(InRaw, height)
	if err != nil {
		return
	}
...
//按宽度进行比例缩放，输入输出都是文件
func ScaleF2FH(filename string, savepath string, height int) (err error) {
	raw, err := ioutil.ReadFile(filename)
	if err != nil {
		return err
	}

	out, err := ScaleB2Bh(raw, height)
	if err != nil {
		return err
	}
...
//对图片字节数组等比例变化,宽度为newdx,返回图像编码和文件类型
func Scaleh(raw []byte, nehex int) (dst *image.RGBA, filetype string, err error) {
	src, filetype, err := LoadImage(bytes.NewReader(raw))
	if err != nil {
		return
	}
	...
	dx := bound.Dx()
	dy := bound.Dy()
	dst = image.NewRGBA(image.Rect(nehex*dx/dy, nehex, 0, 0))
